...
fn validate_args(args: &UpdateSpecArgs) -> Result<()> {
    ...
    if !matches!(args.file_type.to_lowercase().as_str(), "spec" | "task-list" | "tasks" | "notes") {
        return Err(anyhow::anyhow!(
...
fn get_current_content(project_name: &str, spec_name: &str, file_type: &SpecFileType) -> Result<String> {
    let file_path = get_file_path(project_name, spec_name, file_type)?;
    
    filesystem::read_file(&file_path)
        .or_else(|_| Ok(String::new())) // Return empty string if file doesn't exist or can't be read
}

/// Get the file path for a specific spec file type
fn get_file_path(project_name: &str, spec_name: &str, file_type: &SpecFileType) -> Result<std::path::PathBuf> {
...
fn generate_next_steps(args: &UpdateSpecArgs) -> Vec<String> {
    vec![
        format!("Successfully updated {} for spec '{}'", args.file_type, args.spec_name),
        format!("Content {} with {} operation", 
            if args.operation == "append" { "appended" } else { "replaced" },
            args.operation
        ),
        format!("Load updated spec: foundry load-spec {} {}", args.project_name, args.spec_name),
        format!("Continue iterating: foundry update-spec {} {} --operation append", args.project_name, args.spec_name),
...
fn generate_workflow_hints(args: &UpdateSpecArgs, content_length: usize) -> Vec<String> {
    ...
    match args.file_type.to_lowercase().as_str() {
        ...
        "spec" => {
            ...
            hints.push("Include Requirements, Acceptance Criteria, and Implementation sections".to_string());
        }
        "notes" => {
