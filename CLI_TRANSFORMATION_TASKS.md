...
- [ ] Test installation from crates.io: `cargo install foundry`
...
- Use `cargo build` and test with `./target/debug/foundry`
...
- Development: `./target/debug/foundry` or `./target/release/foundry`
- Local install: `~/.cargo/bin/foundry` (from `cargo install --path .`)
- Published install: `~/.cargo/bin/foundry` (from `cargo install foundry`)
