...
impl LazySpecificationContent {
    ...
    pub async fn get_content(&self) -> Result<&String> {
        self.content
            .get_or_try_init(|| async { self.load_content().await })
            .await
    }
    ...
    pub async fn get_tasks(&self) -> Result<&Vec<Task>> {
        self.tasks
            .get_or_try_init(|| async { self.load_tasks().await })
            .await
    }
    ...
    pub async fn get_notes(&self) -> Result<&Vec<Note>> {
        self.notes
            .get_or_try_init(|| async { self.load_notes().await })
            .await
    }
    ...
    pub async fn preload_all(&self) -> Result<()> {
        ...
        let (content_result, tasks_result, notes_result) =
            tokio::join!(self.get_content(), self.get_tasks(), self.get_notes());
    ...
    async fn load_content(&self) -> Result<String> {
        ...
        if let Some(cached_content) = self.cache.get_file_content(&cache_key) {
            tracing::debug!(
                "Retrieved spec content for '{}:{}' from cache",
                self.project_name,
                self.spec_id
            );
        ...
        tracing::debug!(
            "Loading spec content for '{}:{}' from filesystem",
            self.project_name,
            self.spec_id
        );
    ...
    async fn load_tasks(&self) -> Result<Vec<Task>> {
        ...
        if let Some(cached_content) = self.cache.get_file_content(&cache_key) {
            tracing::debug!(
                "Retrieved task list for '{}:{}' from cache",
                self.project_name,
                self.spec_id
            );
        ...
        tracing::debug!(
            "Loading task list for '{}:{}' from filesystem",
            self.project_name,
            self.spec_id
        );
        ...
        self.cache
            .cache_file_content(&cache_key, task_list_content.clone());
    ...
    async fn load_notes(&self) -> Result<Vec<Note>> {
        ...
        if let Some(cached_content) = self.cache.get_file_content(&cache_key) {
            tracing::debug!(
                "Retrieved notes for '{}:{}' from cache",
                self.project_name,
                self.spec_id
            );
        ...
        tracing::debug!(
            "Loading notes for '{}:{}' from filesystem",
            self.project_name,
            self.spec_id
        );
        ...
        // Cache the raw content
        self.cache
            .cache_file_content(&cache_key, notes_content.clone());
...
impl LazyLoadingManager {
    ...
    pub fn new(fs_manager: FileSystemManager, cache: ProjectManagerCache) -> Self {
        Self { cache, fs_manager }
    }

    /// Create a lazy specification content loader
    pub fn create_spec_loader(
        &self,
        project_name: &str,
        spec_id: &str,
...
mod tests {
    ...
    async fn test_background_preloading() {
        ...
        manager
            .preload_specs_background(&project_name, &spec_ids)
            .await;
