...
async fn test_project_creation_and_loading() {
    ...
    assert!(loaded_project.is_ok(), "Failed to load project: {:?}", loaded_project);
...
async fn test_specification_creation() {
    ...
    let spec_result = spec_repo.create_spec(
        &project_name,
        spec_name,
        spec_description,
        spec_content
    ...
    assert!(spec_result.is_ok(), "Failed to create specification: {:?}", spec_result);
...
async fn test_task_management() {
    ...
    let add_result = spec_repo.add_task(&project_name, &spec.id, task.clone()).await;
    ...
    // Update task status
    let update_result = spec_repo.update_task_status(
        &project_name,
        &spec.id,
        &task.id,
    ...
    let updated_task_list = spec_repo.load_task_list(&project_name, &spec.id).await.unwrap();
...
async fn test_notes_management() {
    ...
    let add_result = spec_repo.add_note(&project_name, &spec.id, note.clone()).await;
...
async fn test_list_operations() {
    ...
    spec_repo.create_spec(&project_name1, "feature_a", "Feature A", "# Feature A").await.unwrap();
    spec_repo.create_spec(&project_name1, "feature_b", "Feature B", "# Feature B").await.unwrap();
