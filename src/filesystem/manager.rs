        ...
        profile_operation!("fs_write_file_safe", {
            ...
            if file_path.exists() {
                profile_operation!("fs_create_backup", {
                    ...
                    fs::copy(file_path, &backup_path).map_err(|e| {
                        errors::helpers::file_system_error("create backup", file_path, e)
                    })
                })?;
            }
            ...
            profile_operation!("fs_write_temp_file", {
                ...
                let mut temp_file = File::create(&temp_path).map_err(|e| {
                    errors::helpers::file_system_error("create temp file", &temp_path, e)
                })?;

                temp_file.write_all(content.as_bytes()).map_err(|e| {
                    errors::helpers::file_system_error("write to temp file", &temp_path, e)
                })?;
                temp_file.flush().map_err(|e| {
                    errors::helpers::file_system_error("flush temp file", &temp_path, e)
                })?;
...
mod tests {
    ...
    fn test_new_fs_manager() {
        ...
        assert!(fs_manager.base_dir.ends_with(".foundry"));
    }
    ...
    fn test_write_file_creates_backup() {
        ...
        // Write original file
        fs_manager
            .write_file_safe(&file_path, original_content)
            .unwrap();
    ...
    fn test_write_large_content() {
        ...
        fs_manager
            .write_file_safe(&file_path, &large_content)
            .unwrap();
        let read_content = fs_manager.read_file(&file_path).unwrap();
