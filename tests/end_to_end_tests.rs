...
async fn test_complete_project_development_workflow() {
    ...
    for (spec_name, description) in feature_specs {
        let spec = spec_repo.create_spec(
            ...
            spec_name,
            description,
            &format!("# {}\n\nDetailed specification for {}.", description, spec_name)
        ).await.unwrap();
    ...
    for (i, spec) in created_specs.iter().enumerate() {
        let tasks = match i {
            0 => vec![ // user_authentication
                ("Design authentication flow", "High", "Design JWT-based authentication system"),
                ("Implement user registration", "High", "Create user signup endpoint with validation"),
                ("Implement user login", "High", "Create login endpoint with JWT token generation"),
                ("Add password reset", "Medium", "Implement forgot password functionality"),
            ],
            1 => vec![ // api_endpoints
                ("Design API schema", "High", "Create OpenAPI specification"),
                ("Implement user endpoints", "High", "CRUD operations for user management"),
                ("Implement data endpoints", "Medium", "Business logic API endpoints"),
                ("Add API documentation", "Low", "Generate and host API documentation"),
            ],
            2 => vec![ // frontend_components
                ("Setup React project", "High", "Initialize React app with TypeScript"),
                ("Create authentication components", "High", "Login, register, logout components"),
                ("Build dashboard components", "Medium", "Main application dashboard"),
                ("Implement responsive design", "Medium", "Mobile-friendly UI components"),
            ],
            3 => vec![ // database_schema
                ("Design user tables", "High", "User accounts and profiles schema"),
                ("Design application tables", "High", "Core business logic tables"),
                ("Add database migrations", "Medium", "Version-controlled schema changes"),
                ("Optimize database queries", "Low", "Add indexes and query optimization"),
            ],
            4 => vec![ // deployment_pipeline
                ("Setup CI/CD pipeline", "High", "GitHub Actions or GitLab CI"),
                ("Configure Docker containers", "High", "Containerize application components"),
                ("Setup monitoring", "Medium", "Application and infrastructure monitoring"),
                ("Add automated testing", "Medium", "Unit, integration, and e2e tests"),
            ],
            _ => continue,
        };
        ...
        for (title, priority_str, description) in tasks {
            ...
            let task = Task {
                id: format!("task_{}_{}", spec.id, title.to_lowercase().replace(" ", "_")),
            ...
            spec_repo.add_task(&project_name, &spec.id, task).await.unwrap();
        }
    }
    ...
    let notes_data = vec![
        ("user_authentication", vec![
            ("Consider OAuth 2.0 for third-party login", NoteCategory::Implementation),
            ("Use bcrypt for password hashing", NoteCategory::Decision),
            ("Implement rate limiting for login attempts", NoteCategory::Enhancement),
        ]),
        ("api_endpoints", vec![
            ("Follow REST conventions strictly", NoteCategory::Decision),
            ("Add comprehensive input validation", NoteCategory::Implementation),
            ("Consider GraphQL for complex queries", NoteCategory::Question),
        ]),
        ("frontend_components", vec![
            ("Use Material-UI for consistent design", NoteCategory::Decision),
            ("Implement lazy loading for performance", NoteCategory::Enhancement),
            ("Add accessibility features (ARIA)", NoteCategory::Implementation),
        ]),
    ];
    ...
    for (spec_name, note_list) in notes_data {
        ...
        for (content, category) in note_list {
            ...
            spec_repo.add_note(&project_name, &spec.id, note).await.unwrap();
        }
    }
    
    // Step 6: Simulate development progress by updating task statuses
    let auth_spec = created_specs.iter().find(|s| s.name == "user_authentication").unwrap();
    let task_list = spec_repo.load_task_list(&project_name, &auth_spec.id).await.unwrap();
    
    // Mark first two tasks as completed
    for (i, task) in task_list.tasks.iter().take(2).enumerate() {
        let new_status = if i == 0 { TaskStatus::Completed } else { TaskStatus::InProgress };
        spec_repo.update_task_status(&project_name, &auth_spec.id, &task.id, new_status).await.unwrap();
    }
    ...
    for spec in &all_specs {
        ...
        let task_list = spec_repo.load_task_list(&project_name, &spec.id).await.unwrap();
    ...
    let api_spec = created_specs.iter().find(|s| s.name == "api_endpoints").unwrap();
    let mut updated_spec = spec_repo.load_spec(&project_name, &api_spec.id).await.unwrap();
    updated_spec.status = SpecStatus::Completed;
    
    spec_repo.update_spec(&project_name, &updated_spec).await.unwrap();
    
    let completed_spec = spec_repo.load_spec(&project_name, &api_spec.id).await.unwrap();
...
async fn test_project_pause_resume_workflow() {
    ...
    let initial_tasks = vec![
        ("Setup project structure", TaskStatus::Completed, TaskPriority::High),
        ("Implement core logic", TaskStatus::InProgress, TaskPriority::High),
    ...
    spec_repo.update_spec(&project_name, &paused_spec).await.unwrap();
    
    // Add a note about pausing
    let pause_note = Note {
        id: "pause_note".to_string(),
        content: "Project paused due to priority changes. Current progress: core logic 70% complete.".to_string(),
        category: NoteCategory::Decision,
        created_at: Utc::now(),
    };
    spec_repo.add_note(&project_name, &spec.id, pause_note).await.unwrap();
    ...
    let current_tasks = spec_repo.load_task_list(&project_name, &spec.id).await.unwrap();
    ...
    spec_repo.update_spec(&project_name, &active_spec).await.unwrap();
    
    // Continue work - complete next task
    let in_progress_task = current_tasks.tasks.iter()
        .find(|t| t.status == TaskStatus::InProgress)
        .unwrap();
    
    spec_repo.update_task_status(
        &project_name,
        &spec.id,
        &in_progress_task.id,
    ...
    // Start next task
    let next_task = current_tasks.tasks.iter()
    ...
    let resume_note = Note {
        ...
        content: "Project resumed. Completed core logic implementation. Starting error handling.".to_string(),
        category: NoteCategory::Implementation,
        created_at: Utc::now(),
    };
    spec_repo.add_note(&project_name, &spec.id, resume_note).await.unwrap();
    
    // Verify final state
    let final_tasks = spec_repo.load_task_list(&project_name, &spec.id).await.unwrap();
    let completed_tasks: Vec<_> = final_tasks.tasks.iter()
        .filter(|t| t.status == TaskStatus::Completed)
        .collect();
    let in_progress_tasks: Vec<_> = final_tasks.tasks.iter()
